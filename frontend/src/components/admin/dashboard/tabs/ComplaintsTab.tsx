          ...
          <div className="flex w-full md:w-auto bg-[#0a0f1d]/40 p-1.5 rounded-[1.25rem] border border-white/[0.08] backdrop-blur-xl shadow-2xl overflow-x-auto no-scrollbar">
            {['All', 'Open', 'In Review', 'Resolved'].map((s) => (
              <button
                ...
                className={cn(
                  "flex-1 md:flex-none px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-500 whitespace-nowrap",
                  filterStatus === s 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
         {[
           ...
           <motion.div
             ...
             className={cn("p-5 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] border border-white/[0.05] backdrop-blur-3xl group relative overflow-hidden transition-all duration-500 hover:border-white/10", stat.bg)}
           ...
           >
              ...
              <p className="text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 group-hover:text-slate-400 transition-colors relative z-10">{stat.label}</p>
              <p className={cn("text-2xl sm:text-4xl font-black italic tracking-tighter relative z-10", stat.color)}>{stat.value}</p>
           </motion.div>
         ))}
      </div>
           ...
           <AnimatePresence mode="popLayout">
             {filteredComplaints.map((c, index) => (
               ...
               >
                  ...
                  <div className="flex flex-col lg:flex-row gap-8 sm:gap-14 relative z-10">
                     
                     {/* Visual Intel thumbnail */}
                     <div className="w-full lg:w-52 h-64 lg:h-52 rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden border border-white/10 shrink-0 bg-slate-950 flex items-center justify-center shadow-2xl group/img relative">
                        {c.imageUrl ? (
                          <>
                     ...
                     <div className="flex-1 space-y-6 sm:space-y-8">
                        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                           ...
                           {c.status === 'Resolved' && c.updatedAt && (
                              <motion.div 
                                ...
                                className="flex items-center gap-3 text-emerald-500 bg-emerald-500/5 px-4 py-2 sm:px-5 sm:py-2.5 rounded-xl sm:rounded-2xl border border-emerald-500/20 shadow-lg shadow-emerald-500/10 w-fit"
                              >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-8">
                           ...
                           <div className="space-y-2">
                              ...
                              <div className="flex items-center justify-between p-3 sm:p-4 bg-white/[0.03] rounded-xl sm:rounded-2xl border border-white/[0.05] group/item hover:bg-white/[0.05] transition-all">
                                 <span className="text-xs sm:text-sm font-black text-cyan-400 uppercase italic tracking-tight">{c.category}</span>
                                 <ArrowRight className="size-3.5 text-cyan-400/20 group-hover/item:translate-x-1 transition-transform" />
                              </div>
                           </div>
                        </div>
                        ...
                        <div className="space-y-3">
                           ...
                           <div className="p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2.5rem] bg-white/[0.03] border border-white/[0.08] relative group/msg overflow-hidden">
                              <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent pointer-events-none" />
                              <MessageSquare className="absolute -top-4 -left-4 size-10 sm:size-14 text-white/5 group-hover/msg:text-cyan-400/10 transition-colors" />
