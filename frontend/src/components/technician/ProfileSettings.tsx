    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20">
      ...
      >
        <div className="absolute top-0 right-0 p-8 opacity-10">
          <Shield className="size-40 text-indigo-600" />
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
          ...
          <div className="text-center md:text-left flex-1">
            <h2 className="text-4xl font-black text-slate-900 tracking-tighter mb-2">Identity Settings</h2>
            <p className="text-slate-500 font-bold uppercase tracking-[0.2em] text-xs">Manage your professional credentials and presence</p>
          ...
          <div className="flex flex-col gap-3">
            <button 
              ...
              className="px-6 py-3 rounded-2xl bg-white border border-slate-200 text-slate-500 font-bold text-xs uppercase tracking-widest hover:bg-slate-50 transition active:scale-95"
            >
              Exit Protocol
            </button>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-10">
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    <User className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-slate-400 group-focus-within:text-indigo-600 transition" />
                    <input 
                      name="name"
                      value={formData.name}
                      onChange={handleChange}
                      className="w-full bg-slate-50/50 border border-slate-200 rounded-[1.5rem] py-5 pl-16 pr-6 text-slate-900 font-bold placeholder:text-slate-300 focus:bg-white focus:border-indigo-500 transition-all outline-none shadow-inner"
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group">
                    <Phone className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-slate-400 group-focus-within:text-indigo-600 transition" />
                    <input 
                      name="phone"
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  <Wrench className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-slate-400 group-focus-within:text-indigo-600 transition z-10" />
                  ...
                  <select 
                    ...
                    className="w-full bg-slate-50/50 border border-slate-200 rounded-[1.5rem] py-5 pl-16 pr-12 text-slate-900 font-bold focus:bg-white focus:border-indigo-500 transition-all outline-none shadow-inner appearance-none cursor-pointer"
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  <MapPin className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-slate-400 group-focus-within:text-indigo-600 transition" />
                  <input 
                    ref={addressInputRef}
                    name="address"
              ...
              <div className="space-y-3">
                ...
                <div className="relative group">
                  <Lock className="absolute left-6 top-1/2 -translate-y-1/2 size-5 text-slate-400 group-focus-within:text-indigo-600 transition" />
                  <input 
                    name="passwordHint"
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mr-4">
                ...
                {selectedServices.size > 0 && (
                  <div className="flex flex-wrap gap-2 p-4 bg-slate-50/50 border border-slate-200 rounded-2xl">
                    {Array.from(selectedServices).map(svc => (
                      <span key={svc} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-[11px] font-bold">
                        {svc}
              ...
              <button 
                onClick={handleSave}
                disabled={saving}
                className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] text-xs shadow-2xl shadow-slate-900/20 hover:bg-indigo-600 transition-all active:scale-[0.98] flex items-center justify-center gap-3 disabled:opacity-50"
              >
                {saving ? (
        ...
        <div className="space-y-8">
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            className="glass-neon-card p-8 bg-indigo-600 text-white border-none shadow-indigo-500/20"
          >
          ...
          >
            <div className="flex items-center gap-3 mb-6">
               ...
               <h3 className="text-sm font-black text-rose-600 uppercase tracking-widest">Critical Action Zone</h3>
            </div>
            <p className="text-rose-900/60 font-bold text-[10px] leading-relaxed uppercase tracking-widest mb-8">Terminating your account is irreversible. All history, earnings, and credentials will be purged.</p>
            <button 
              onClick={() => setShowDeleteConfirm(true)}
              className="w-full py-4 bg-white border border-rose-200 text-rose-500 font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl hover:bg-rose-500 hover:text-white transition-all active:scale-95 shadow-sm"
            >
              Terminate Identity
            </button>
          </motion.div>
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md glass-neon-card p-12 bg-white border-none shadow-2xl overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-rose-600 to-orange-600" />
              
              <div className="size-20 rounded-[2rem] bg-rose-50 border border-rose-100 flex items-center justify-center mx-auto mb-8">
                <ShieldAlert className="size-10 text-rose-500" />
              </div>
              
              <h3 className="text-2xl font-black text-slate-900 text-center mb-4 tracking-tight uppercase">Confirm Termination?</h3>
              <p className="text-slate-500 text-center text-xs font-bold leading-relaxed uppercase tracking-widest mb-10">
              ...
              <div className="flex flex-col gap-4">
                ...
                <button
                  onClick={() => setShowDeleteConfirm(false)}
                  disabled={deleting}
                  className="w-full py-5 bg-slate-100 text-slate-500 font-black text-xs uppercase tracking-[0.2em] rounded-[1.5rem] hover:bg-slate-200 transition active:scale-95"
                >
                  Cancel Protocol
                </button>
              </div>
