      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <Link href="/" className="group">
          <Logo 
            ...
            textClassName="text-white group-hover:text-indigo-400"
          />
        </Link>
        ...
        <div className="flex items-center gap-6">
            ...
            {(user || customProfile) ? (
              <div className="flex items-center gap-4">
                <div className="relative">
                  <button 
                    ...
                    className="relative p-2.5 text-white bg-indigo-500/10 border border-indigo-400/20 hover:text-indigo-400 hover:bg-indigo-500/20 rounded-xl transition cursor-pointer"
                  >
                     <Bell className="size-5" />
                     <span className="absolute top-2.5 right-2.5 size-2 bg-rose-500 rounded-full border border-indigo-950" />
                  </button>
                        ...
                        >
                          ...
                          <div className="p-3 space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
                            <div className="p-3 bg-white/5 border border-white/10 rounded-xl">
                              <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">System Update</p>
                              <p className="text-xs font-medium text-white">Advanced AI scan is active. Proceed with service booking.</p>
                            </div>
                            <div className="p-3 bg-white/5 border border-white/10 rounded-xl">
                              <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest mb-1">Account Sync</p>
                   ...
                   <div className="relative group/profile">
                      <button className="flex items-center gap-3 group">
                        <div className="size-10 rounded-2xl overflow-hidden border border-white/20 group-hover:border-indigo-400 transition shadow-sm">
                           {profile?.avatar && profile.avatar.length > 5 ? (
                             ...
                             <div className="size-full bg-indigo-500/10 flex items-center justify-center text-lg text-indigo-400 font-black">{profile?.name ? profile.name.charAt(0).toUpperCase() : '👤'}</div>
                           )}
                        </div>
                        <div className="hidden md:block text-left">
                           ...
                           {profile?.role && <p className="text-[9px] text-indigo-300 font-black uppercase tracking-wider mt-0.5">{profile.role}</p>}
                        </div>
                      ...
                      <div className="absolute top-full right-0 mt-4 w-60 bg-[#0d0d1b] border border-white/20 rounded-[2rem] shadow-2xl opacity-0 translate-y-4 pointer-events-none group-hover/profile:opacity-100 group-hover/profile:translate-y-0 group-hover/profile:pointer-events-auto transition-all duration-500 z-50 overflow-hidden">
                        <div className="p-3 space-y-1">
                          <Link href={`/${profile?.role}/dashboard`} className="flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-indigo-200 hover:text-white hover:bg-white/10 rounded-2xl transition">
                            <LayoutDashboard className="size-4 text-indigo-400" /> Control Panel
                          </Link>
                          <Link href={`/${profile?.role}/dashboard#profile`} className="flex items-center gap-3 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-indigo-200 hover:text-white hover:bg-white/10 rounded-2xl transition">
              ...
              <div className="hidden lg:flex items-center gap-5">
                ...
                <Link href="/auth/signup" className={cn(
                  "px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl bg-indigo-500 text-white hover:bg-indigo-400 shadow-indigo-500/30"
                )}>
                  Register
                </Link>
              </div>
          ...
          <button 
            ...
            className={cn(
              "lg:hidden size-10 flex items-center justify-center rounded-xl border transition-colors bg-indigo-500/10 border-indigo-400/20 text-indigo-200 hover:bg-indigo-500/20 hover:text-white"
            )}
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={cn(
              "lg:hidden overflow-hidden mt-4 bg-indigo-950/40 backdrop-blur-2xl border border-indigo-400/20 rounded-[2.5rem] shadow-[0_8px_32px_rgba(79,70,229,0.2)]"
            )}
          >
        ...
        {showAboutModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center px-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-indigo-950/40 backdrop-blur-xl border border-indigo-500/30 rounded-[3rem] shadow-[0_0_30px_rgba(79,70,229,0.3)] overflow-hidden"
            >
              ...
              <div className="p-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-400/30 text-indigo-300 text-[10px] font-black uppercase tracking-widest mb-6 shadow-[0_0_15px_rgba(79,70,229,0.2)]">
                      <Shield className="size-3.5 text-indigo-400" /> Admin Protocol
                    </div>
                    <h2 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-indigo-200 to-cyan-300 tracking-tight">System Identity</h2>
                  </div>
                  <button 
                    onClick={() => setShowAboutModal(false)}
                    className="p-3 text-indigo-200 hover:text-cyan-300 hover:bg-indigo-500/10 border border-transparent hover:border-indigo-500/20 rounded-2xl transition duration-300"
                  >
                    <X className="size-6" />
                  </button>
                </div>
                ...
                <div className="space-y-8">
                  {loadingAdmin ? (
                    <div className="flex flex-col items-center justify-center py-16 space-y-6">
                      <div className="size-10 border-4 border-indigo-500/20 border-t-cyan-400 rounded-full animate-spin shadow-[0_0_15px_rgba(34,211,238,0.4)]" />
                      <p className="text-indigo-300 text-[10px] font-black uppercase tracking-[0.2em] animate-pulse">Syncing Admin Records...</p>
                    </div>
                  ) : (
                    <div className="bg-indigo-950/50 rounded-[2rem] p-8 border border-indigo-500/20 space-y-8 backdrop-blur-sm shadow-[inset_0_0_20px_rgba(79,70,229,0.1)]">
                      <div className="flex items-center gap-5">
                        <div className="size-16 rounded-2xl bg-indigo-500/10 border border-indigo-400/20 flex items-center justify-center shadow-[0_0_15px_rgba(79,70,229,0.1)] transition-all hover:border-indigo-400/40">
                          <User className="size-8 text-indigo-400" />
                        </div>
                        <div>
                          <p className="text-[9px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">Assigned Liaison</p>
                      ...
                      <div className="space-y-6">
                        <div className="flex items-start gap-4">
                          <div className="mt-1 p-2 rounded-xl bg-indigo-500/10 border border-indigo-400/20 text-indigo-400 transition-all hover:border-indigo-400/40 hover:shadow-[0_0_10px_rgba(79,70,229,0.2)]">
                            <LayoutDashboard className="size-4" />
                          </div>
                          <div>
                            <p className="text-[9px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">Organization</p>
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <p className="text-[9px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">Deployment Zone</p>
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <p className="text-[9px] font-black text-indigo-300 uppercase tracking-[0.2em] mb-1">Communication Channels</p>
                            <p className="text-white font-bold text-sm tracking-tight">{adminDetails?.phone || 'Encrypted'}</p>
                            <p className="text-cyan-300 font-bold text-sm tracking-tight hover:text-cyan-200 transition">{adminDetails?.email || 'admin@fixnow.app'}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="text-center pt-4">
                    <p className="text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">© {new Date().getFullYear()} <span className="notranslate">FIXNOW</span> Service Ecosystem</p>
                  </div>
                </div>
              </div>
            </motion.div>
  ...
  return (
    <Link 
      ...
      className={cn(
        ...
        active 
          ? (isLanding ? "text-white" : "text-indigo-600")
          : (isLanding ? "text-slate-400 hover:text-white" : "text-slate-500 hover:text-indigo-600")
      )}
    >
      {children}
    </Link>
