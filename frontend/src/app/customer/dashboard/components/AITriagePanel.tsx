...
export default function AITriagePanel({
  ...
  return (
    ...
    >
      {analyzing && (
        <div className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-md border border-white/20 flex items-center justify-center">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg sm:text-xl font-black text-white flex items-center gap-3">
            <Sparkles className="w-5 h-5 text-white" /> Advanced AI Scan & Diagnose
          </h2>
          ...
          <div className="flex items-center justify-between px-2 min-h-[44px]">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 bg-white/5 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-white/10 hover:border-white/30 hover:bg-white/10 transition shadow-sm h-11 active:scale-95"
              >
                ...
                {showLangMenuVoice && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 top-full mt-2 w-48 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-y-auto max-h-60 z-50 py-2 custom-scrollbar"
                    >
                      {langs.map((l) => (
                        <button
                          key={l.code}
          ...
          <motion.button
            ...
            className={cn(
              ...
              isListening 
                ...
                : "bg-white/5 border-white/10 hover:border-white/30 hover:bg-white/10 shadow-inner"
            )}
          >
            <div className={cn(
              ...
              isListening 
                ...
                : "bg-white/5 border-white/10 text-slate-400 group-hover:scale-110 group-hover:bg-white/10 group-hover:border-white/30 group-hover:text-white"
            )}>
              <Mic className="w-8 h-8" />
            </div>
            <div className="flex-1">
              <h3 className="text-sm font-black text-white group-hover:text-white transition uppercase tracking-wider">
                {isListening ? 'Listening your voice...' : 'Speak Issue'}
              </h3>
              <p className="text-xs text-slate-400 font-medium mt-0.5 leading-relaxed group-hover:text-slate-200">Describe the issue out loud to diagnose instantly</p>
            </div>
          </motion.button>
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between px-2 h-[34px]">
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-white/40" /> Visual Diagnosis:
            </span>
          </div>
          <input
            type="file"
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-center">
              <div className="flex items-center justify-between">
                ...
                {imagePreview && (
                  <motion.div 
                    ...
                    className="relative group w-10 h-10 rounded-xl overflow-hidden border border-white/20 shadow-md cursor-pointer ml-3 flex-shrink-0"
                  >
              ...
              <p className="text-xs text-slate-400 font-medium mt-0.5 leading-relaxed group-hover:text-slate-200">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Or Type Description Manually</h4>
          <div className="h-[1px] flex-1 bg-slate-800/40 backdrop-blur-md ml-4" />
        </div>

        <textarea
          className="w-full h-36 rounded-[1.75rem] border border-white/10 bg-white/5 px-8 py-6 text-white placeholder:text-slate-500 focus:outline-none focus:border-white/30 transition text-md leading-relaxed shadow-inner font-medium resize-none"
      ...
      <div className="flex flex-wrap justify-end items-center gap-6">
        <button
          ...
          className="px-6 sm:px-10 py-4 sm:py-5 rounded-[1.5rem] font-black text-[10px] uppercase tracking-[0.2em] text-slate-900 bg-white shadow-2xl shadow-black/20 hover:bg-slate-100 disabled:opacity-50 transition-all active:scale-95 flex items-center gap-3 relative z-20 w-full sm:w-auto justify-center"
        >
          {analyzing ? (
            <>
